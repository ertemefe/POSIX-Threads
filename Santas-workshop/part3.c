...
pthread_mutex_t lockTime, lockFile;
pthread_mutex_t lockNzQue;
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    NzPaintingQue = ConstructQueue(1000);
    NzAssemblyQue = ConstructQueue(1000);
    NzQAQue = ConstructQueue(1000);
    NzPackagingQue = ConstructQueue(1000);
    NzDeliveryQue = ConstructQueue(1000);
    ...
    pthread_mutex_init(&lockNzQue, NULL);
    ...
    pthread_mutex_destroy(&lockNzQue);
...
void *ElfA(void *arg)
{
    ...
    while (TRUE)
    {
        ...
        pthread_mutex_lock(&lockNzQue);
        if (!isEmpty(NzPackagingQue))
        {
            *nextTask = Dequeue(NzPackagingQue);
            pthread_mutex_unlock(&lockNzQue);
            ...
            // Lock before enque
            pthread_mutex_lock(&lockNzQue);
            Enqueue(NzDeliveryQue, *newTask);
            pthread_mutex_unlock(&lockNzQue);
            continue;
        }

        // Check if PaintingQue is empty
        if (!isEmpty(NzPaintingQue))
        {
            // Deque next Painting task
            *nextTask = Dequeue(NzPaintingQue);
            pthread_mutex_unlock(&lockNzQue);
            ...
            if (nextTask->GiftType == 4)
            {
                ...
                if (packagingCanBeDone[tempGiftID] == 2)
                {
                    ...
                    pthread_mutex_lock(&lockNzQue);
                    Enqueue(NzPackagingQue, *newTask);
                    pthread_mutex_unlock(&lockNzQue);
                    // free(nextTask);
                    continue;
                }
...
void *ElfB(void *arg)
{
    while (TRUE)
    {
        ...
        // Check if PaintingQue is empty
        if (!isEmpty(NzAssemblyQue))
        {
            // Deque next Painting task
            *nextTask = Dequeue(NzAssemblyQue);
            pthread_mutex_unlock(&lockNzQue);
...
void *Santa(void *arg)
{
    while (TRUE)
    {
        ...
        pthread_mutex_lock(&lockNzQue);
        if (!isEmpty(NzDeliveryQue))
        {
            *nextTask = Dequeue(NzDeliveryQue);
            pthread_mutex_unlock(&lockNzQue);
        ...
        if (!isEmpty(QAQue))
        {
            if ((QAQue->size > 2) || isEmpty(DeliveryQue))
            {
                // Deque next Painting task
                *nextTask = Dequeue(QAQue);
                pthread_mutex_unlock(&lockQue);
    ...
    while (TRUE)
    {
        ...
        if ((giftType >= 0) && (giftType <= 9))
        {
            ...
            if (NzGift)
            {
                ...
                pthread_mutex_lock(&lockNzQue);
                Enqueue(NzPackagingQue, *TaskChoclate);
        ...
        else if ((giftType >= 10) && (giftType <= 13))
        {
            pthread_mutex_lock(&lockID);
            int tempTaskID = taskID;
            taskID++;
        ...
        else if ((giftType >= 14) && (giftType <= 17))
        {
            pthread_mutex_lock(&lockID);
            int tempTaskID = taskID;
            taskID++;
            ...
            if (NzGift)
            {
                ...
                // Enqueue(PackagingQue, *TaskChoclate);
                Enqueue(NzAssemblyQue, *TaskPlasticT);
                // Exit Critical section
                pthread_mutex_unlock(&lockNzQue);
            }
            else
        ...
        else if (giftType == 18)
        {
            ...
            if (NzGift)
            {
                ...
                // Enqueue(PackagingQue, *TaskChoclate);
                Enqueue(NzPaintingQue, *TaskWoodenT);
                Enqueue(NzQAQue, *TaskGameS);
                // Exit Critical section
                pthread_mutex_unlock(&lockNzQue);
            }
            else
        ...
        else if (giftType == 19)
        {
            ...
            if (NzGift)
            {
                ...
                // Enqueue(PackagingQue, *TaskChoclate);
                Enqueue(NzPaintingQue, *TaskPlasticT);
                Enqueue(NzQAQue, *TaskGameS);
                // Exit Critical section
                pthread_mutex_unlock(&lockNzQue);
            }
            else
            {
                ...
                // Enqueue(PackagingQue, *TaskChoclate);
                Enqueue(PaintingQue, *TaskPlasticT);
                Enqueue(QAQue, *TaskGameS);
                // Exit Critical section
                pthread_mutex_unlock(&lockQue);
            }
...
void printQueues(int currTime)
{
    ...
    pthread_mutex_lock(&lockNzQue);
    // Painting Queue
    printf("At %d sec New Zealand painting  : ", currTime);
    if ((NzPaintingQue->size) > 0)
    {
        int leng = NzPaintingQue->size;
        int j;
        id;

        elem = NzPaintingQue->head;
        p = &(elem->data);
        id = p->ID;
        printf(" %d", id);
    ...
    // Assembly Queue
    printf("At %d sec New Zealand assembly  : ", currTime);
    if ((NzAssemblyQue->size) > 0)
    {
        int leng = NzAssemblyQue->size;
        int j;
        id;

        elem = NzAssemblyQue->head;
        p = &(elem->data);
        id = p->ID;
        printf(" %d", id);
    ...
    printf("At %d sec New Zealand packaging  : ", currTime);
    if ((NzPackagingQue->size) > 0)
    {
        int leng = NzPackagingQue->size;
        int j;
        id;

        elem = NzPackagingQue->head;
        p = &(elem->data);
        id = p->ID;
        printf(" %d", id);
    ...
    // Delivery Queue
    printf("At %d sec New Zealand delivery  : ", currTime);
    if ((NzDeliveryQue->size) > 0)
    {
        int leng = NzDeliveryQue->size;
        int j;
        id;

        elem = NzDeliveryQue->head;
        p = &(elem->data);
        id = p->ID;
        printf(" %d", id);
    ...
    // QA Queue
    printf("At %d sec New Zealand QA  : ", currTime);
    if ((NzQAQue->size) > 0)
    {
        int leng = NzQAQue->size;
        int j;
        id;
